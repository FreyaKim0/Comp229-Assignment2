...
module.exports.processAddPage = (req, res, next) => {
  ...
  // 檢查前端的line有順利存到嗎? 有
  // 但是後端的array=[] 長度是0
  for (let i = 0; i < req.body.cart.lines.length; i++) {
    let book = new Book(
      req.body.cart.lines[i].book._id,
      req.body.cart.lines[i].book.name,
      req.body.cart.lines[i].author,
      req.body.cart.lines[i].published,
      req.body.cart.lines[i].description,
      req.body.cart.lines[i].price,
      req.body.cart.lines[i].store,
      req.body.cart.lines[i].__v,
      req.body.cart.lines[i].originalPrice
    );
    let quantity = req.body.cart.lines.quantity;
    cart.lines.push({ book: book, quantity: 777 });
  }
