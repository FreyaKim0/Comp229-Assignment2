...
module.exports.processEditPage = (req, res, next) => {
  ...
  for (let i = 0; i < req.files.length; i++) {
    ...
    fileArray.push({
      generateName: element.filename,
    });
  }

  const imagePath0 =
    "http://localhost:3500/server/images/" + fileArray[0].generateName;
  const imagePath1 =
    "http://localhost:3500/server/images/" + fileArray[1].generateName;
  const imagePath2 =
    "http://localhost:3500/server/images/" + fileArray[2].generateName;
  const imagePath3 =
    "http://localhost:3500/server/images/" + fileArray[3].generateName;
  const imagePath4 =
    "http://localhost:3500/server/images/" + fileArray[4].generateName;
  const imagePath5 =
    "http://localhost:3500/server/images/" + fileArray[5].generateName;
  const x = req.body.price;
  const price = Number(x);
  const y = req.body.originalPrice;
  const originalPrice = Number(y);
